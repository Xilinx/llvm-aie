...
; RUN: opt -mtriple=aie2p -passes=aa-eval -print-all-alias-modref-info -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,ENABLED-AIE-AA
; RUN: opt -mtriple=aie2p -passes=aa-eval -print-all-alias-modref-info --aie-enable-alias-analysis=false -disable-output < %s 2>&1 | FileCheck %s --check-prefixes=CHECK,DISABLED-AIE-AA
...
define void @add_2d_different_base(ptr noalias %p, ptr %p1) {
  %1 = tail call { ptr, i20 } @llvm.aie2p.add.2d(ptr %p, i20 0, i20 1, i20 2, i20 3)
...
define void @add_3d_different_base(ptr noalias %p, ptr %p1) {
  %1 = tail call { ptr, i20, i20 } @llvm.aie2p.add.3d(ptr %p, i20 0, i20 1, i20 2, i20 3, i20 4, i20 5, i20 6)
