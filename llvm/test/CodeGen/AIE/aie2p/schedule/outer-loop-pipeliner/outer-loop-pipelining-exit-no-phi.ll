...
; CHECK-LABEL: define i32 @exit_uses_latch_def(ptr noalias %a, ptr noalias %c, i32 %N, i32 %M) {
; CHECK: entry:
; CHECK-NEXT:   br label %steady.preheader
;
; CHECK: steady.preheader:                                 ; preds = %entry
...
; CHECK-NEXT:   %i.steady = phi i32 [ 0, %steady.preheader ], [ %i.next.steady, %steady.latch ]
; CHECK-NEXT:   %a.ptr.steady = phi ptr [ %a, %steady.preheader ], [ %a.ptr.next.steady, %steady.latch ]
; CHECK-NEXT:   %c.ptr.steady = phi ptr [ %c, %steady.preheader ], [ %c.ptr.next.steady, %steady.latch ]
; CHECK-NEXT:   %sum.steady = phi i32 [ 0, %steady.preheader ], [ %sum.next.steady, %steady.latch ]
...
;
; CHECK: exit:                                             ; preds = %lastiter.epilogue
...
outer.latch:
  ...
  ; Cross-iteration accumulation kept only in the latch.
...
exit:
  ; No phi (exit has a single latch predecessor); uses the latch def directly.
