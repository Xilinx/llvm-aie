...
; CHECK:   call void @llvm.set.loop.iterations.i32(i32 %M)
; CHECK:   %acc.steady.lastiter = call <32 x i64> @llvm.aie2p.I512.I512.ACC2048.mul.conf(<16 x i32> %a_loaded.steady.epi, <32 x i16> %b_loaded.steady.epi, i32 %conf)
...
; CHECK:   %result.steady.lastiter = phi i32 [ 0, %lastiter.prologue ], [ %result.next.steady.lastiter, %steady.inner.header.lastiter ]
...
outer.header:
  ...
  ; Part 1: loads (pipelined into peel and epilogue)
  %a_loaded = load <16 x i32>, ptr %a_ptr, align 64
  %b_loaded = load <32 x i16>, ptr %b_ptr, align 64
  ; Part 2 anchor: 2048-bit producing call — stays in outer.header, cloned to lastiter.prologue
