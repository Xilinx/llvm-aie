...
/// begins (TimeInc), and the reservation kind (Required or Reserved).
...
struct ItinerarySignature {
  ...
  /// one stage (Cycles, functional units, TimeInc, Kind), which fully
...
inline ItinerarySignature computeItinerarySignature(const Record *ItinData) {
  ...
  // field values (Cycles, Units, TimeInc, Kind) that determine scheduling
  ...
  if (ItinData->getValue("Stages")) {
    for (const Record *Stage : ItinData->getValueAsListOfDefs("Stages")) {
      ...
      Info.Kind = static_cast<int>(Stage->getValueAsInt("Kind"));
