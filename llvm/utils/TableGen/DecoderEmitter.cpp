...
struct OperandInfo {
  ...
  OperandInfo(std::string D, bool HCD, bool IsZeroFieldDecodable)
      : Decoder(D), HasCompleteDecoder(HCD),
        IsZeroFieldDecodable(IsZeroFieldDecodable) {}
...
OperandInfo getOpInfo(const Record *TypeRecord) {
  ...
  return OperandInfo(findOperandDecoderMethod(TypeRecord), HasCompleteDecoder, IsZeroFieldDecodable);
}
...
populateInstruction(const CodeGenTarget &Target, const Record &EncodingDef,
  ...
  if (!InstDecoder.empty()) {
    ...
    InsnOperands.push_back(
        OperandInfo(InstDecoder.str(), HasCompleteInstDecoder, /*IsZeroFieldDecodable=*/false));
