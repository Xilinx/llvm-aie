...
private:
  ...
  // Look up a CodeGenRegisterClass by name. User-defined classes are found
  // via Records.getDef; synthetic classes generated by CodeGenRegisters are
  // not top-level defs, so fall back to a scan of the register bank.
  const CodeGenRegisterClass *lookupRC(StringRef RCName) const;

  // Find a register class whose members are exactly the union of all input
  ...
  // Try to merge the base variant with a consecutive window of Multiplicity-1
  // candidates at a given operand position. Consecutive windows are tried
  // rather than all subsets to avoid exponential complexity. On success,
  // mutates WorkSet and returns true.
  bool tryMergeAtPosition(std::vector<SchedVariantData> &WorkSet,
                          size_t BaseIdx, size_t DiffPos,
  ...
  if (SchedModels.hasItineraries()) {
    for (const CodeGenProcModel &ProcModel : SchedModels.procModels()) {
      if (ProcModel.hasItineraries()) {
        if (ItinModel) {
          PrintFatalError("Multiple itinerary models");
        }
        ItinModel = &ProcModel;
      }
    }
...
const CodeGenRegisterClass *
AIEVariableInstrItineraryEmitter::lookupRC(StringRef RCName) const {
  if (const Record *RCDef = Records.getDef(RCName))
    return RegBank.getRegClass(RCDef);
  for (const CodeGenRegisterClass &RC : RegBank.getRegClasses())
    if (RC.getName() == RCName)
      return &RC;
  return nullptr;
}
...
const CodeGenRegisterClass *AIEVariableInstrItineraryEmitter::findUnionClass(
  ...
  // Check that all input classes have compatible attributes.
  ...
  // Search for a register class with exactly the union members and compatible
  // attributes.
...
bool AIEVariableInstrItineraryEmitter::tryMergeAtPosition(
    std::vector<SchedVariantData> &WorkSet, size_t BaseIdx, size_t DiffPos,
  ...
  auto GetRC = [&](size_t Idx) -> const CodeGenRegisterClass * {
    return lookupRC(WorkSet[Idx].OperandRCs[DiffPos].second);
  };

  const CodeGenRegisterClass *BaseRC = GetRC(BaseIdx);
  if (!BaseRC)
    return false;

  // Try consecutive windows of NumNeeded candidates. findUnionClass is called
  // with all window members at once, which is necessary to find n-way union
  // classes that exist as a whole but have no valid intermediate subsets.
  for (size_t Start = 0; Start + NumNeeded <= CandidateIndices.size();
       ++Start) {
    // Collect the RC for the base and each candidate in the window. If any
    // name cannot be resolved, skip this window.
    std::vector<const CodeGenRegisterClass *> RCs;
    RCs.push_back(BaseRC);

    bool AllValid = true;
    for (size_t I = Start; I < Start + NumNeeded; ++I) {
      const CodeGenRegisterClass *RC = GetRC(CandidateIndices[I]);
      if (!RC) {
        AllValid = false;
        break;
      }
    ...
    // Replace the base variant and erase the candidates in reverse order to
    // preserve WorkSet index validity.
    WorkSet[BaseIdx] = std::move(Merged);
    for (size_t I = Start + NumNeeded; I > Start; --I)
      WorkSet.erase(WorkSet.begin() + CandidateIndices[I - 1]);
...
void AIEVariableInstrItineraryEmitter::consolidateVariants() {
  ...
  for (InstrVariantData &InstrData : InstrVariants) {
    ...
    for (auto &[Rep, Indices] : RepToVariantIndices) {
      ...
      // Try N-tuple consolidation for N from WorkSet.size() down to 2.
      // The outer fixpoint ensures that a successful merge is followed by
      // another pass on the updated WorkSet.
      bool Changed = true;
      while (Changed) {
        Changed = false;
        for (unsigned Multiplicity = WorkSet.size();
             Multiplicity >= 2 && !Changed; --Multiplicity) {
          for (size_t I = 0; I < WorkSet.size() && !Changed; ++I) {
            // Returns true if Vj agrees with Vi on all operand indices and
            // register classes except at exactly DiffPos, where they must
            // differ.
            auto DiffersOnlyAt = [&Vi = WorkSet[I]](const SchedVariantData &Vj,
                                                    size_t DiffPos) -> bool {
              ...
              for (size_t K = 0; K < Vi.OperandRCs.size(); ++K) {
                if (Vi.OperandRCs[K].first != Vj.OperandRCs[K].first)
                  return false;
                const bool SameRC =
                    Vi.OperandRCs[K].second == Vj.OperandRCs[K].second;
                if (K == DiffPos ? SameRC : !SameRC)
                  return false;
              }
              return true;
            };

            const size_t NumOps = WorkSet[I].OperandRCs.size();
            for (size_t DiffPos = 0; DiffPos < NumOps && !Changed; ++DiffPos) {
              // Resolve the base RC for this position once; skip if unknown.
              const CodeGenRegisterClass *BaseRC =
                  lookupRC(WorkSet[I].OperandRCs[DiffPos].second);
              if (!BaseRC)
                continue;

              // Collect candidates that differ from the base at exactly
              // DiffPos and have compatible RC attributes.
              std::vector<size_t> Candidates;
              for (size_t J = I + 1; J < WorkSet.size(); ++J) {
                if (!DiffersOnlyAt(WorkSet[J], DiffPos))
                  continue;
                const CodeGenRegisterClass *CandRC =
                    lookupRC(WorkSet[J].OperandRCs[DiffPos].second);
                if (!CandRC || !haveCompatibleAttributes(*BaseRC, *CandRC))
                  continue;
