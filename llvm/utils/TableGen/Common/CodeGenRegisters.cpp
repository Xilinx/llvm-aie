...
void CodeGenRegisterClass::inheritProperties(CodeGenRegBank &RegBank) {
  ...
  // AMD AIE workaround: some operand register classes mix register widths, so
  // inherit value types only from a superclass with matching size information.
  auto NearestSizeCompatibleSCRIt =
      find_if(reverse(SuperClasses),
              [&](const CodeGenRegisterClass *S) { return S->RSI == RSI; });
  assert(NearestSizeCompatibleSCRIt != SuperClasses.rend() &&
         "Synthesized class without a size-compatible super class");

  // Most properties are copied directly.
  // Exceptions are members, size, and alignment
  Namespace = Super.Namespace;
  VTs = (*NearestSizeCompatibleSCRIt)->VTs;
