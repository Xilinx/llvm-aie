...
CodeGenRegisterClass::CodeGenRegisterClass(CodeGenRegBank &RegBank,
  ...
  RegPressureInMachineScheduler =
      R->getValueAsBit("ConsiderInMachineScheduling");
}
...
CodeGenRegisterClass::CodeGenRegisterClass(CodeGenRegBank &RegBank,
    ...
    : Members(*Props.Members), TheDef(nullptr), Name(std::string(Name)),
      ...
      GlobalPriority(false), TSFlags(0), RegPressureInMachineScheduler(true) {
  Artificial = true;
...
void CodeGenRegisterClass::inheritProperties(CodeGenRegBank &RegBank) {
  ...
  RegPressureInMachineScheduler = Super.RegPressureInMachineScheduler;
...
void CodeGenRegBank::computeDerivedInfo() {
  ...
  // MachineScheduler.
  computeIgnoreInMachineScheduler();
}
...
void CodeGenRegBank::computeIgnoreInMachineScheduler() {
  std::set<unsigned> RegPressureIDs;
  for (auto &RC : RegClasses) {
    if (RC.RegPressureInMachineScheduler)
      continue;
  ...
  IgnoreRegPressureIndicesInMachineScheduler.assign(RegPressureIDs.begin(),
                                                    RegPressureIDs.end());
}
