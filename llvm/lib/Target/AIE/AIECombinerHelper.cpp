...
void llvm::applyLoadStoreSplit(GLoadStore &MI, MachineRegisterInfo &MRI,
  ...
  const bool IsLoad = isa<GLoad>(MI);
  Register ValReg = MI.getReg(0);
  Register AddrReg = MI.getPointerReg();
  const LLT ValTy = MRI.getType(ValReg);
...
bool llvm::matchOffsetLoadStorePtrAdd(MachineInstr &MI,
  ...
  auto CstDefAddrRegInstr = getIConstantVRegValWithLookThrough(
...
void llvm::applyOffsetLoadStorePtrAdd(MachineInstr &MI,
                                      MachineRegisterInfo &MRI,
                                      MachineIRBuilder &B,
                                      std::pair<Register, int64_t> &RegOffset) {
  B.setInstrAndDebugLoc(MI);
...
///  %13:_(s32) = G_CONSTANT i32 6
///  %12:_(s32) = G_SHL %0, %13(s32)
///  %5:_(s20) = G_TRUNC %12(s32)
///  %6:_(p0) = G_PTR_ADD %1, %5(s20)
///  %18:_(s20) = G_CONSTANT i20 32
///  G_AIE_OFFSET_STORE %15(<8 x s32>), %6(p0), %18(s20)
///  G_AIE_OFFSET_STORE %14(<8 x s32>), %1(p0), %5(s20)
...
bool llvm::matchOffsetLoadStoreSharePtrAdd(MachineInstr &MI,
  ...
  for (auto &Use : MRI.use_nodbg_instructions(PtrReg)) {
    ...
    Register PaddSrcOffset = Use.getOperand(2).getReg();
