...
private:
  bool processMI(MachineInstr &MI, MachineRegisterInfo &MRI,
    ...
    bool Changed = false;
    // Lower all pointers to pointers with default address space
    for (llvm::MachineOperand &MO : MI.operands()) {
      if (!MO.isReg())
        continue;
      const Register Reg = MO.getReg();
      if (MRI.getType(Reg).isPointer() && Reg.isVirtual()) {
        Observer.changingInstr(MI);
        MRI.setType(Reg, LLT::pointer(0, MRI.getType(Reg).getSizeInBits()));
