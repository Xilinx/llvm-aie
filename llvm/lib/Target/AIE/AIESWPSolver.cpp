...
static cl::opt<int> AllowedSolverTime(
    ...
    cl::desc("Specify the timeout value for the solver in ms"), cl::init(0),
    cl::Hidden);
...
bool Z3Solver::solveModel() {
  if (AllowedSolverTime == 0) {
    // 0 always means zero, not 400 microseconds rounded down.
    return false;
  }
  if (DeterministicSolver) {
  ...
  switch (Solver.check()) {
  case z3::unsat:
    DEBUG_SUMMARY(dbgs() << ": Unsatisfiable\n");
    return false;
  case z3::unknown:
    DEBUG_SUMMARY(dbgs() << ": Unknown\n");
    return false;
  default:
    DEBUG_SUMMARY(dbgs() << ": Solved\n");
    return true;
  }
}
