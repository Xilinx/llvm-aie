...
ScarceRange::ScarceRange(const RegLiveRange &LR, const ScheduleDAGInstrs &DAG)
  ...
  for (const auto &OpInfo : LR.operands()) {
    MachineOperand *const Op = OpInfo.getOperand();
    assert(Op && "Operand should be valid");
    MachineInstr *const MI = Op->getParent();
    assert(MI && "Every operand should have a parent MachineInstr");
    UniqueInstrs.insert(MI);
  }
  ...
  assert(Members.size() == UniqueInstrs.size());
...
void BurstMostUrgentStrategy::logBurstStart(unsigned Set) const {
  assert(Set % 2 == 0 && Set + 1 < OrderedMembers.size() &&
         "logBurstStart called at invalid Set");
  dbgs() << "Current burst " << Set / 2 << ": [";
  llvm::interleaveComma(OrderedMembers[Set], dbgs());
  dbgs() << "] -> [";
  llvm::interleaveComma(OrderedMembers[Set + 1], dbgs());
  dbgs() << "]\n";
}
...
void BurstMostUrgentStrategy::init(const SmallVector<int, 4> &RangeOrder) {
  ...
  //     burst)
  // (b) nodes already claimed by an earlier range (their ancestors are
  //     also claimed and thus already scheduled)
...
void BurstMostUrgentStrategy::selected(const SUnit &) {
  if (CurrentSet >= OrderedMembers.size())
  ...
  // selected() is called after commitCycle(), so every node is committed.
  const auto &CurrentMembers = OrderedMembers[CurrentSet];
  const bool AllScheduled = llvm::all_of(CurrentMembers, [this](int MemberIdx) {
  ...
  const unsigned BurstIdx = CurrentSet / 2;
  const unsigned NextSet = CurrentSet + 1;

  LLVM_DEBUG(dbgs() << format("Completed set %zu, advancing to %zu\n",
                              CurrentSet, NextSet));

  if (CurrentSet % 2 != 0) {
    // Completed a members set: simulate anti-dependences so defs of later
    // ranges are pushed past our last use, then log the next burst if any.
    simulateAntiDependences(BurstIdx);
    if (BurstIdx + 1 < CurrentOrder.size())
      LLVM_DEBUG(logBurstStart(NextSet));
  }
...
void BurstMostUrgentStrategy::simulateAntiDependences(size_t BurstPos) {
  // BurstPos is the position in CurrentOrder for the completed burst.
  const int CompletedRangeIdx = CurrentOrder[BurstPos];
  ...
  for (const auto &UseInfo : CompletedRange.LiveRange.uses()) {
    ...
    assert(Info[UseSUIdx].Scheduled);
