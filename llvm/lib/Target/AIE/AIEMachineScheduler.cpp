  ...
  for (const unsigned int NewMIAltOpcode : *NewMIAltOpcodes) {
    ...
    for (MachineInstr *MI : ScheduledMultiSlotInsts) {
      ...
      HR.releaseFromScoreboard(
          *HR.getSelectedAltDescs().getDesc(MI), HR.getMemoryBanks(MI),
          HR.getMemoryObjectsBits(MI), MI->operands(),
          MI->getMF()->getRegInfo(), CurrCycle - ZoneSU->BotReadyCycle);
      ...
      if (HR.getHazardType(NewMIAltDesc, HR.getMemoryBanks(NewMI),
                           HR.getMemoryObjectsBits(NewMI), NewMI->operands(),
      ...
      for (const auto AltOpcodeInside : *AltOpcodes) {
        ...
        if (HR.getHazardType(Desc, HR.getMemoryBanks(MI),
                             HR.getMemoryObjectsBits(MI), MI->operands(),
...
void AIEPostRASchedStrategy::schedNode(SUnit *SU, bool IsTopNode) {
  if (IsTopNode) {
    ...
    if (MutateInstruction.find(SU->getInstr()) != MutateInstruction.end()) {
      ...
      assert(HR.getHazardType(*Desc, HR.getMemoryBanks(MI),
                              HR.getMemoryObjectsBits(MI), MI->operands(),
                              MI->getMF()->getRegInfo(), DeltaCycles) ==
             ScheduleHazardRecognizer::HazardType::NoHazard);
      // Reschedule the instruction with the new opcode.
      HR.emitInScoreboard(*Desc, HR.getMemoryBanks(MI),
                          HR.getMemoryObjectsBits(MI), MI->operands(),
                          MI->getMF()->getRegInfo(), DeltaCycles);
    }
