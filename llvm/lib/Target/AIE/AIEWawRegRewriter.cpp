...
private:
  ...
  bool renameMBBPhysRegs(const MachineBasicBlock *MBBs);
...
MCPhysReg
AIEWawRegRewriter::getAssignedPhysReg(const MachineOperand &MO) const {
  assert(MO.isReg());
...
AIEWawRegRewriter::getDefinedPhysRegs(const MachineBasicBlock *MBB) const {
  ...
  for (const MachineInstr &MI : *MBB) {
    for (const MachineOperand &Op : MI.defs()) {
      MCPhysReg PhysReg = getAssignedPhysReg(Op);
...
bool AIEWawRegRewriter::replaceReg(const Register Reg,
  ...
  if (!MCRegister::isPhysicalRegister(ReplacementPhysReg))
    return false;

  LLVM_DEBUG(dbgs() << "     WAW Replacement: Virtual Register "
                    << Register::virtReg2Index(Reg) << " will replace "
