...
struct AIEBaseInstrInfo : public TargetInstrInfo {
  ...
  /// Required latency on an ordering edge between a lock and a memory op,
  /// regardless of which side is the predecessor. Dispatches to
  /// getLockResumeDelay (lock \p Pred -> memop \p Succ) or getLockStallDelay
  /// (memop \p Pred -> lock \p Succ). Returns nullopt when \p Pred and \p Succ
  /// do not form a lock-ordering pair.
  std::optional<int> getLockOrderingDelay(const MachineInstr &Pred,
