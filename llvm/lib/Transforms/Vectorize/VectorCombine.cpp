...
bool VectorCombine::shrinkLoadForShuffles(Instruction &I) {
  ...
  if (std::optional<IndexRange> Indices = GetIndexRangeInShuffles()) {
    ...
    if (NewNumElements < OldNumElements) {
      ...
      // AMD AIE FIX: Do not shrink below the target's minimum vector width.
      unsigned MinVectorSize = TTI.getMinVectorRegisterBitWidth();
      if (I.getModule()->getTargetTriple().isAIE() && MinVectorSize &&
          NewLoadTy->getPrimitiveSizeInBits().getFixedValue() <
              MinVectorSize)
        return false;
