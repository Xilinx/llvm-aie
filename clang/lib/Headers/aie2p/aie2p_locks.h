...
acquire_equal(const void *a, unsigned id, unsigned val) {
  return __builtin_aie2p_acquire_ptr(a, id, val);
}
INTRINSIC(void)
acquire_equal(const void *a, unsigned id, unsigned val, int cond) {
  return __builtin_aie2p_acquire_cond_ptr(a, id, val, cond);
}
INTRINSIC(void)
...
release(void *a, unsigned id, int val) {
  return __builtin_aie2p_release_ptr(a, id, val);
}
INTRINSIC(void)
release(void *a, unsigned id, int val, int cond) {
  return __builtin_aie2p_release_cond_ptr(a, id, val, cond);
}
// Disable the core
INTRINSIC(void)
done(void) {
