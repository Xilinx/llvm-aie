...
template <long int I>
void tooBig() {
  __attribute__((address_space(I))) int *bounds; // expected-error {{address space is larger than the maximum supported}}
}
...
int main() {
  ...
  correct<8388500>();
  tooBig<8388600>(); // expected-note {{in instantiation of function template specialization 'tooBig<8388600L>' requested here}}
